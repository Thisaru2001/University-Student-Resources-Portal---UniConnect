...
document.addEventListener("DOMContentLoaded", function () {
  ...
  webSearchBtn.addEventListener('click', function () {
    ...
    this.querySelector('.btn-text').textContent = webSearchMode ? 'Web Search ON' : 'Web Search';
    showToast(webSearchMode ? '🌐 Web search enabled for next message' : 'Web search disabled');
  });
}
...
async function sendMessage() {
  ...
  // Create AI placeholder with typing dots / thinking indicator
  const msg = document.createElement("div");
  msg.className = "message ai";
  ...
  const selectedModel = document.getElementById("modelSelect")?.value || "qwen3.5:4b";
...
let requestBody = {
    ...
    stream: false,
    options: {
        temperature: deepThinkMode ? 0.5 : 0.7,
        num_predict: deepThinkMode ? 800 : 300,
        think: false
    },                          
    ...
    contentDiv.textContent = ""; // clear placeholder

    function typeWord() {
      if (index < words.length) {
        displayed += (index === 0 ? "" : " ") + words[index];
        contentDiv.textContent = displayed;
        chatMessages.scrollTop = chatMessages.scrollHeight;
        index++;
    ...
    // Still show time even on error (if startTime was captured)
    const endTime = performance.now();
    const elapsedSeconds = (endTime - startTime) / 1000;
    const timeSpan = document.createElement("span");
    timeSpan.className = "response-time";
